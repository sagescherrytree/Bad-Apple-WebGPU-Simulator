...
export class VelocityGrid {
    ...
    //     // --- Debug compute pass: write solid red into velocityA ---
    //     const debugShaderModule = device.createShaderModule({
    //         code: `
    //     @group(0) @binding(0) var velocityA : texture_storage_2d<rgba32float, write>;

    //     @compute @workgroup_size(8,8)
    //     fn main(@builtin(global_invocation_id) id : vec3<u32>) {
    //         if (id.x >= ${this.width}u || id.y >= ${this.height}u) { return; }
    //         textureStore(velocityA, vec2<i32>(id.xy), vec4<f32>(1.0, 0.0, 0.0, 1.0));
    //     }
    //     `
    //     });
    ...
    //         layout: "auto",
    //         compute: { module: debugShaderModule, entryPoint: "main" }
    //     });

    //     const bindGroup = device.createBindGroup({
    //         layout: debugPipeline.getBindGroupLayout(0),
    //         entries: [
    //             { binding: 0, resource: this.velocityA.createView({ dimension: "2d" }) }
    //         ]
    //     });
